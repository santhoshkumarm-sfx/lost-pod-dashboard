...
export function createSupabaseServerClient(): SupabaseClient {
  if (process.env.AUTH_DISABLED === "true") {
    // Public-access mode: there's no visitor session for RLS to scope
    // against, so page reads/writes go through the service-role client
    // instead. This is what makes every visitor see everything — see
    // README "Temporary public access" before enabling AUTH_DISABLED.
 ...
 *   - createSupabaseServerClient()'s AUTH_DISABLED branch, above
 ...
 *
 * Uses a static import (not require()) so TypeScript keeps proper types
 * flowing through — a dynamic require() here previously made this (and
 * therefore createSupabaseServerClient()'s return type) collapse to `any`
 * everywhere it was used, silently erasing type-checking on every page.
 */
